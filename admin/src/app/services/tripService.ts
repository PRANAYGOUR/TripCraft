...
import type { Trip as AdminTrip } from '../types/admin';
// For getAllTrips, we return AdminTrip which has admin-specific fields
...
class TripService {
   ...
   */
  async getAllTrips(): Promise<ApiResponse<AdminTrip[]>> {
    try {
      const currentUser = await authService.getCurrentUser();
      ...
      const formattedTrips = (data || []).map((trip: any) => ({
        ...trip,
        customerName: trip.profiles?.name || 'Unknown Customer',
        customerEmail: trip.profiles?.email || '',
        checkInDate: trip.start_date,
        checkOutDate: trip.end_date,
        submittedDate: trip.created_at,
        numberOfPeople: trip.travelers,
      }));

      return {
        success: true,
        data: formattedTrips as AdminTrip[],
      };
    } catch (error) {
      return {
        success: false,
  ...
  /**
   * UPDATE TRIP - Generic method for updating any trip fields
   * Used by admin to update status, approved hotel, and notes
   */
